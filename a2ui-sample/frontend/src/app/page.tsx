...
export default function Home() {
  ...
  const [protocolLog, setProtocolLog] = useState<
    Array<{ time: string; data: any }>
  >([]);
  const [showConsole, setShowConsole] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  const processor = useMemo(() => {
    return new MessageProcessor([customCatalogInstance], (action) => {
            ...
            for (const chunk of chunks) {
              ...
              if (dataLine) {
                try {
                  ...
                  setProtocolLog((log) => [
                    ...log,
                    { time: new Date().toISOString().slice(11, 23), data },
                  ]);
      ...
      >
        ...
        >
          ...
          <p style={{ fontSize: 12, color: "#64748b", margin: "4px 0 0" }}>
            Official @a2ui/react v0.9 + Strands + Bedrock
          </p>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
          ...
          {loading && (
            <div style={{ fontSize: 13, color: "#6366f1" }}>Processing...</div>
          )}
          ...
          >
            <div style={{ textAlign: "center" }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 500 }}>
                A2UI-rendered interface appears here
              </p>
              <p style={{ fontSize: 12 }}>
                Using official @a2ui/react v0.9 renderer
              </p>
            </div>
          </div>
        )}
          ...
          {showConsole && (
            ...
            >
              ...
              >
                ...
                >
                  📡 A2UI Protocol ({protocolLog.length})
                </span>
                <button
